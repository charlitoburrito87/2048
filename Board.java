...
/**
 * Board contains methods that change, test, and print 2048 boards.
 *
 * Bugs: None known
 *
...
public class Board {
    ...
    */
    ...
    /**
     * Prints the current board as a 2D grid.
     */
    public void printBoard() {
      int numRows = this.GRID_SIZE;
      int numCols = this.GRID_SIZE;

      for (int r = 0; r < numRows; r++) {
        for (int c = 0; c < numCols; c++) {
          int number = this.grid[r][c];
    ...
    private boolean canMoveUp() {
      for (int c = 0; c < this.GRID_SIZE; c++) {
        ...
        //returns true if there is an empty space above a numbered tile
        for (int r = 0; r < this.GRID_SIZE; r++) {
          if (this.grid[r][c] == 0) {
            zeroIndex = r;
            if ((zeroIndex != this.GRID_SIZE - 1) && zeroIndex != -1) {
              for (int row = zeroIndex + 1; row < this.GRID_SIZE; row++) {
        ...
        //returns true if two values can be combined by moving up
        int nonZero = -1;
        for (int r = 0; r < this.GRID_SIZE; r++) {
          if (this.grid[r][c] != 0) {
            if (this.grid[r][c] == nonZero) {
              return true;
            }
            nonZero = this.grid[r][c];
          }
        }
      }
        return false;
    ...
    private boolean canMoveDown() {
      for (int c = 0; c < this.GRID_SIZE; c++) {
        ...
        //returns true if there is an empty space below a numbered tile
        for (int r = this.GRID_SIZE - 1; r > -1; r--) {
          if (this.grid[r][c] == 0) {
            zeroIndex = r;
            if ((zeroIndex != 0) && zeroIndex != -1) {
              for (int row = zeroIndex - 1; row > -1; row--) {
                if (this.grid[row][c] != 0) {
                  return true;
                }
              }
        ...
        //returns true if two values can be combined by moving down
        int nonZero = -1;
        for (int r = this.GRID_SIZE - 1; r > -1; r--) {
          if (this.grid[r][c] != 0) {
            if (this.grid[r][c] == nonZero) {
    ...
    private boolean canMoveRight() {
      for (int r = 0; r < this.GRID_SIZE; r++) {
        ...
        //returns true if there is an empty space to the right of a number tile
        for (int c = this.GRID_SIZE - 1; c > -1; c--) {
          if (this.grid[r][c] == 0) {
            zeroIndex = c;
            if ((zeroIndex != 0) && zeroIndex != -1) {
              for (int col = zeroIndex - 1; col > -1; col--) {
        ...
        //returns true if two values can be combined by moving right
        int nonZero = -1;
        for (int c = this.GRID_SIZE - 1; c > -1; c--) {
          if (this.grid[r][c] != 0) {
            if (this.grid[r][c] == nonZero) {
    ...
    private boolean canMoveLeft() {
      for (int r = 0; r < this.GRID_SIZE; r++) {
        ...
        //returns true if there is an empty space to the left of a number tile
        for (int c = 0; c < this.GRID_SIZE; c++) {
          if (this.grid[r][c] == 0) {
            zeroIndex = c;
            if ((zeroIndex != this.GRID_SIZE - 1) && zeroIndex != -1) {
              for (int col = zeroIndex + 1; col < this.GRID_SIZE; col++) {
                if (this.grid[r][col] != 0) {
                  return true;
                }
              }
        ...
        //returns true if two values can be combined by moving left
        int nonZero = -1;
        for (int c = 0; c < this.GRID_SIZE; c++) {
          if (this.grid[r][c] != 0) {
            if (this.grid[r][c] == nonZero) {
     ...
     */
    private void moveUp() {
      //combines pairs that are adjacent or have zeros in between them
      for (int c = 0; c < this.GRID_SIZE; c++) {
        int nonZero = -1;
        int nonZeroIndex = -1;
        for (int r = 0; r < this.GRID_SIZE; r++) {
          if (this.grid[r][c] != 0) {
            if (this.grid[r][c] == nonZero) {
              this.grid[nonZeroIndex][c] = nonZero * 2;
              this.grid[r][c] = 0;
              nonZero = -1;
              nonZeroIndex = -1;
            }
      ...
      for (int c = 0; c < this.GRID_SIZE; c++) {
        ...
        for (int r = 0; r < this.GRID_SIZE; r++) {
          if (this.grid[r][c] == 0) {
            ...
            this.grid[r - numZeros][c] = this.grid[r][c];
            this.grid[r][c] = 0;
          }
        }
      }
    ...
    private void moveLeft() {
      ...
      for (int r = 0; r < this.GRID_SIZE; r++) {
        ...
        for (int c = 0; c < this.GRID_SIZE; c++) {
          if (this.grid[r][c] != 0) {
            if (this.grid[r][c] == nonZero) {
              this.grid[r][nonZeroIndex] = nonZero * 2;
              this.grid[r][c] = 0;
              nonZero = -1;
              nonZeroIndex = -1;
            }
      ...
      for (int r = 0; r < this.GRID_SIZE; r++) {
        ...
        for (int c = 0; c < this.GRID_SIZE; c++) {
          if (this.grid[r][c] == 0) {
            ...
            this.grid[r][c - numZeros] = this.grid[r][c];
            this.grid[r][c] = 0;
          }
        }
      }
